// Destaque de sintaxe muito simples para alguns idiomas
const HL = {
  js: {
...
// Aplica o destaque de sintaxe a todos os blocos <pre><code>
export function highlightAll(root=document){
  root.querySelectorAll('pre code').forEach(codeEl=>{
    const lang=(codeEl.className.match(/lang-([\w-]+)/)||[])[1]||'js';
    const L=HL[lang]||HL.js;
    let s=codeEl.textContent;
    s=s.replace(L.c,m=>`<span class='tok-c'>${esc(m)}</span>`);
